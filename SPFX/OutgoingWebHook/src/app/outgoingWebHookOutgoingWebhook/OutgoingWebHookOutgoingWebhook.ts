...
/**
 * Implementation for OutgoingWebHook Outgoing Webhook
 */
@OutgoingWebhookDeclaration("/api/webhook")
export class OutgoingWebHookOutgoingWebhook implements IOutgoingWebhook {
    ...
    public requestHandler(req: express.Request, res: express.Response, next: express.NextFunction) {
        ...
        if (securityToken && securityToken.length > 0) {
            ...
            if (msgHash === auth) {
                ...
                message.text = `Echo ${incoming.text} with QQ!!`;
            } else {
